...
JOIN LATERAL
    "$anchor.capsule"\."Point_$anchor.businessName"(timepoints."Time_of_Change"::$schema.metadata.chronon) "p$anchor.mnemonic"
ON
...
AS \$$\$$
SELECT
    $(schema.CRT)? "$anchor.mnemonic"\."$schema.metadata.positorSuffix",
    "$anchor.mnemonic"\."$anchor.identityColumnName" as "$anchor.businessIdentityColumnName",
~*/
        while (attribute = anchor.nextAttribute()) {
            if(attribute.isKnotted()) {
...
RETURNS TABLE (
    $(schema.CRT)? "$schema.metadata.positorSuffix" $schema.metadata.positorRange,
